...
def modificare_judet(cale_fisier):
    df = pd.read_csv(cale_fisier)
    #din ceva motiv in masini.csv Judetul VALCEA este scris VILCEA si atunci am modificat si in
    #array-ul din functia replace pt ca altfel judetul VALCEA/VILCEA nu ar avea schimbarea ceruta 
    df['JUDET'] = df["JUDET"].replace(['DOLJ', 'OLT','GORJ','VILCEA','MEHEDINTI'], 'OLTENIA')
    df.to_csv(cale_fisier, index = False)
    return df
